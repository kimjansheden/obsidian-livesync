...
export async function runSetupURIGenerator(
    ...
    const setupURIFile = requireValue(environment, "setup_uri_file");
    await writeSecretFile(setupURIFile, generated.setupURI);
    if (!environment.uri_passphrase?.trim()) {
        const passphraseFile = requireValue(environment, "uri_passphrase_file");
        await writeSecretFile(passphraseFile, generated.setupPassphrase);
    }
    console.log(`Generated ${generated.remoteType} Setup URI and wrote it to the requested protected file.`);
    console.log("No Setup URI or passphrase was written to the process output.");
}
