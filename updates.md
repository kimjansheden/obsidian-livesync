...
- With an Object Storage remote and **Sync on Startup** enabled, a synchronisation now starts as soon as the app starts or returns to the foreground, instead of one interval later. Applying saved settings does not start one.
- On the mobile app with an Object Storage remote, hiding the app now saves the open editors, stores those saved files directly under their file-event locks, and, when anything waits to be sent, starts a synchronisation before the app is frozen; the plug-in suspends only once it has finished. What cannot be sent then is sent at the next start or return to the app. Nothing is polled while the app is hidden.
- On the mobile app with an Object Storage remote, a replication lock left by a process which the system closed during a synchronisation is taken over at once, so the first synchronisation after the app is started again no longer waits up to 45 seconds for the lock to expire.
- With an Object Storage remote, a file whose content did not change is no longer stored as a new version when only its modification time differs, for example after the start-up scan, or after the plug-in wrote a received version under another time. Successive saves on one device form one chain of versions, also while the start-up scan or a synchronisation runs, instead of versions which conflict with each other. A version from another device which arrives while a file is being saved, or which has arrived but is not yet written to the file, is kept beside the local change as a conflict instead of being replaced by it.
- With an Object Storage remote, two different insertions at the same place, typically successive states of the same typing, are no longer merged automatically by keeping both as extra lines; such a conflict is left for you to resolve. An automatic merge is stored so that devices which merge the same versions create the same version, instead of two merges which conflict with each other.
- A synchronisation cycle with an Object Storage remote sends fewer requests: the synchronisation parameters are read once instead of twice, and the milestone is written only when this device's information in it changes, or every ten minutes, instead of in every cycle.
- The active remote configuration is recreated from the working connection settings when it cannot be decrypted because it was encrypted under an earlier passphrase of the device, so no decryption errors are shown for it at every start. The other configurations are left unchanged.
